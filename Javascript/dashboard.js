...
(function() {
    ...
    const detailPanel = {
        ...
        startRealtimeUpdates: () => {
            ...
            detailPanel.updateInterval = setInterval(() => {
                ...
                document.getElementById('solar-a').textContent = `${solarA.toFixed(1)} MW`;
                document.getElementById('solar-b').textContent = `${solarB.toFixed(1)} MW`;
                document.getElementById('detail-total').textContent = `${(solarA + solarB + utils.random(2.5, 3.5) + utils.random(1.2, 1.8)).toFixed(1)} MW`;

                // Update temps and irradiance
                document.getElementById('temp-a').textContent = `${Math.round(utils.random(32, 38))}°C`;
                document.getElementById('temp-b').textContent = `${Math.round(utils.random(34, 40))}°C`;
                document.getElementById('irr-a').textContent = `${Math.round(utils.random(850, 920))} W/m²`;
                document.getElementById('irr-b').textContent = `${Math.round(utils.random(820, 890))} W/m²`;
                document.getElementById('eff-a').textContent = `${utils.random(20.5, 22).toFixed(1)}%`;
                document.getElementById('eff-b').textContent = `${utils.random(20, 21.5).toFixed(1)}%`;

                // Update wind turbines
                const wind1 = utils.random(2.5, 3.2);
                const wind2 = utils.random(1.2, 1.8);
                document.getElementById('wind-1').textContent = `${wind1.toFixed(1)} MW`;
                document.getElementById('wind-2').textContent = `${wind2.toFixed(1)} MW`;
                document.getElementById('wind-speed-1').textContent = `${Math.round(utils.random(12, 16))} m/s`;
                document.getElementById('wind-speed-2').textContent = `${Math.round(utils.random(8, 12))} m/s`;
                document.getElementById('rotor-1').textContent = `${Math.round(utils.random(16, 20))} RPM`;
                document.getElementById('rotor-2').textContent = `${Math.round(utils.random(10, 14))} RPM`;
                document.getElementById('pitch-1').textContent = `${Math.round(utils.random(10, 15))}°`;
                document.getElementById('pitch-2').textContent = `${Math.round(utils.random(6, 12))}°`;

                // Update grid parameters
                document.getElementById('grid-freq').textContent = `${(50 + utils.random(-0.05, 0.05)).toFixed(2)} Hz`;
    ...
    const liveData = {
        update: () => {
            state.liveGeneration = utils.randomWalk(state.liveGeneration, 0.3, 10, 15);
        ...
        render: () => {
            ...
            document.getElementById('efficiency-value').textContent = `${utils.format(state.efficiency)}%`;
            document.getElementById('storage-value').textContent = `${Math.round(state.storage)}%`;
        }
    };
    ...
    const timeFilters = {
        init: () => {
            ...
            buttons.forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    e.target.classList.add('bg-surface-container-highest', 'text-primary');
                    ...
                    toast.show(`Switched to ${e.target.dataset.period === 'D' ? 'Daily' : e.target.dataset.period === 'W' ? 'Weekly' : 'Monthly'} view`, 'info');
                });
            });
        },
        ...
        updateChart: (period) => {
            ...
            labels.innerHTML = labelSets[period].map(label => `<span>${label}</span>`).join('');
        }
    };
    ...
    const smartTips = {
        ...
        rotate: () => {
            smartTips.currentIndex = (smartTips.currentIndex + 1) % smartTips.tips.length;
        ...
        init: () => {
            ...
            buttons.forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    target.querySelector('.material-symbols-outlined').style.fontVariationSettings = "'FILL' 1";

                    const nav = target.dataset.nav;
                    toast.show(`Navigating to ${nav.charAt(0).toUpperCase() + nav.slice(1)}...`, 'info');
                });
            });
        }
    };
    ...
    const cardInteractions = {
        init: () => {
            document.getElementById('daily-save-card').addEventListener('click', () => {
                toast.show(`Daily energy savings: ${Math.round(state.dailySave)} kWh`, 'info');
            });

            document.getElementById('co2-card').addEventListener('click', () => {
                toast.show(`CO2 offset this month: ${utils.format(state.co2Offset)} tonnes`, 'success');
            });

            document.getElementById('savings-card').addEventListener('click', () => {
                toast.show(`Projected monthly savings: ${utils.formatCurrency(state.projectedSavings)}`, 'info');
            });
        }
    };
