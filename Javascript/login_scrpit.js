...
(function () {
    ...
    function toggleTheme() {
        ...
        showToast(state.isDarkMode ? 'Dark mode enabled' : 'Light mode enabled', 'info', 2000);
    }
    ...
    function validateLogin() {
        ...
        if (!password) {
            ...
            showError('login-password', 'login-password-error', `Minimum ${CONFIG.minPasswordLength} characters`);
            valid = false;
        }

        return valid ? { email, password, remember: document.getElementById('login-remember').checked } : null;
    }
    ...
    function validateRegister() {
        ...
        if (!password || password.length < CONFIG.minPasswordLength) {
            showError('reg-password', 'reg-password-error', `Min ${CONFIG.minPasswordLength} characters`);
            valid = false;
        }
        if (password !== confirm) {
